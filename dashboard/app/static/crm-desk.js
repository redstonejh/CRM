...
(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .crm-overview-frame{position:absolute;inset:var(--crm-canvas-top,78px) var(--crm-canvas-x,64px) var(--crm-canvas-bottom,78px);max-width:1380px;margin:auto;min-height:0}
      .crm-overview-bucket{height:100%;min-width:0;min-height:0;display:grid;grid-template-rows:52px minmax(0,1fr);overflow:hidden;border:1px solid rgba(255,255,255,.14);border-radius:16px;background:transparent;box-shadow:inset 0 1px rgba(255,255,255,.12)}
      .crm-overview-head{display:flex;align-items:center;padding:0 20px;border-bottom:1px solid rgba(255,255,255,.09)}.crm-overview-title{font-size:var(--crm-type-room,17px);font-weight:700}
      .crm-overview-scroll{min-width:0;min-height:0;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;padding:0 20px 20px;scrollbar-width:thin;scrollbar-color:rgba(213,228,249,.36) transparent}
      .crm-overview-scroll::-webkit-scrollbar{width:14px}.crm-overview-scroll::-webkit-scrollbar-track{background:linear-gradient(90deg,transparent 6px,rgba(255,255,255,.09) 6px,rgba(255,255,255,.09) 7px,transparent 7px)}
      .crm-overview-scroll::-webkit-scrollbar-thumb{min-height:48px;border:5px solid transparent;border-radius:999px;background:rgba(213,228,249,.48);background-clip:padding-box}
      .crm-overview-content{min-width:0;min-height:100%;display:grid;grid-template-rows:auto minmax(360px,1fr);gap:18px}
      .crm-overview-project-shelf{min-width:0;min-height:0;display:grid;grid-template-rows:36px 148px;padding-top:2px;border-bottom:1px solid rgba(255,255,255,.07)}
      .crm-overview-section-head{height:36px;display:flex;align-items:center;gap:12px;padding:0 3px}.crm-overview-section-title{font-size:var(--crm-type-body,12px);font-weight:680;color:rgba(255,255,255,.82);text-shadow:0 1px 8px rgba(0,0,0,.72)}
      .crm-overview-empty-inline{color:rgba(255,255,255,.3);font-size:var(--crm-type-meta,10px);font-weight:600}
      ...
      .crm-overview-project-shelf.is-empty{grid-template-rows:38px;padding-top:0}.crm-overview-project-shelf.is-empty .crm-overview-project-list{display:none}
      ...
      .crm-overview-project-name{position:absolute;z-index:2;left:13px;right:13px;bottom:12px;font-size:var(--crm-type-body,12px);font-weight:680;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      ...
      .crm-overview-lower{min-width:0;min-height:0;display:grid;grid-template-columns:minmax(0,1fr) minmax(286px,330px);gap:24px;padding:0 3px;overflow:hidden}
      .crm-overview-ticket-section,.crm-overview-recent{min-width:0;min-height:0;background:transparent}.crm-overview-ticket-section{display:flex;flex-direction:column;justify-content:center}.crm-overview-recent{display:grid;grid-template-rows:36px minmax(0,1fr)}
      .crm-overview-ticket-section>.crm-overview-section-head{flex:0 0 36px}.crm-overview-tickets{min-width:0;min-height:0;flex:0 0 271px;display:flex;align-items:flex-start;gap:16px;overflow-x:auto;overflow-y:hidden;padding:6px 2px 16px;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.17) transparent}
      ...
      .crm-overview-update-title{font-size:var(--crm-type-body,12px);font-weight:650;line-height:1.38}.crm-overview-update-context{margin-top:5px;color:rgba(255,255,255,.45);font-size:var(--crm-type-meta,10px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.crm-overview-update-time{position:absolute;right:2px;top:9px;color:rgba(255,255,255,.38);font-size:var(--crm-type-meta,10px)}
      .crm-overview-empty{min-height:80px;display:grid;place-items:center;color:rgba(255,255,255,.28);font-size:var(--crm-type-caption,11px)}
      @media(max-width:1080px){.crm-overview-scroll{padding-inline:16px}.crm-overview-lower{grid-template-columns:minmax(0,1fr) 250px;gap:18px}.crm-overview-project{flex-basis:224px}.crm-overview-tickets{gap:12px}}
    `;
  ...
  function render() {
    ...
    root.innerHTML = `<div class="crm-overview-frame"><section class="crm-overview-bucket" aria-label="Overview">
      <header class="crm-overview-head"><div class="crm-overview-title">Overview</div></header>
      <div class="crm-overview-scroll"><div class="crm-overview-content">
        <section class="crm-overview-project-shelf${model.projects.length ? "" : " is-empty"}"><header class="crm-overview-section-head"><span class="crm-overview-section-title">Projects</span>${model.projects.length ? "" : '<span class="crm-overview-empty-inline">No projects</span>'}</header>
          <div class="crm-overview-project-list">${model.projects.map((project) => `<button type="button" class="crm-overview-project" data-overview-project="${esc(project.id)}"><span class="crm-overview-project-name">${esc(project.title)}</span>${miniWorld(project)}</button>`).join("")}</div></section>
