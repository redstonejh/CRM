...
Today is now a first-class workspace fed by `/api/reports/summary`. It deals the `todayHand` once per local day with the existing card/detail language: due next touches, scheduled tasks/calendar items, due or overdue invoices, and Cold Front records. Cold Front is derived from `lastTouchAt` and stage-specific half-lives; it desaturates Contacts and Deals and pulls stale Contacts into the People attention deck without storing new state.
