(() => {
  ...
  const ensureTicketStyles = () => {
    ...
    style.textContent = `
      ...
      .widget-card.ticket-widget-card, .widget-card[data-widget-runtime-type="ticket"], .widget-card[data-widget-runtime-type="deal"], .widget-card[data-widget-runtime-type="contact"] {
      ...
      body:not(.panel-interaction-active):not(.panel-resize-active) .widget-card[data-widget-runtime-type="contact"]:not(.td-card):hover {
      ...
      body:not(.panel-interaction-active):not(.panel-resize-active) .widget-card[data-widget-runtime-type="contact"],
      body:not(.panel-interaction-active):not(.panel-resize-active) .widget-card[data-widget-runtime-type="contact"]:hover {
        border: 0 !important;
      }
  ...
  const lastContactByInstance = new Map();
  const contactTouchMs = (contact) => {
  ...
  const contactNeedsAttention = (contact) => {
    const ms = contactTouchMs(contact);
    if (!ms) return true;
    return Date.now() - ms > 1000 * 60 * 60 * 24 * 30;
  };
  const shortContactDate = (ms) => {
    if (!ms) return "No touch";
    try { return new Date(ms).toLocaleDateString([], { month: "short", day: "numeric" }); }
    catch { return ""; }
  };
  const renderContactInto = (mount, contact) => {
    if (!contact) {
      mount.innerHTML = "";
      return;
    }
    ...
    const due = contactNeedsAttention(contact);
    const companyLine = [role, company].filter(Boolean).join(" / ");
    mount.innerHTML = `
      <div class="ticket-company">${escapeHtml(title)}</div>
      ${sub ? `<div class="ticket-host">${escapeHtml(sub)}</div>` : ""}
      ${companyLine ? `<div class="ticket-host">${escapeHtml(companyLine)}</div>` : ""}
      <div class="contact-status" data-attention="${due ? "due" : "current"}">
        <span class="contact-status-dot"></span>
        <span>${escapeHtml(due ? "Needs attention" : shortContactDate(touch))}</span>
      </div>`;
  };
  ...
  registerWidgetDefinition({
    ...
    category: "data",
    widgetType: "contact",
    dashboardObjectKind: "contact",
    regionRole: "content",
    htmlTag: "div",
    className: "widget-card ticket-widget-card contact-widget-card",
    defaultSize: { cols: 1, rows: 3 },
    minSize: { cols: 1, rows: 2 },
    capabilities: { readsContext: true, supportsResize: true },
    supportedSettings: ["color", "pin", "delete"],
    getDefaultConfig: () => ({ title: "Contact" }),
    mountBodyRenderer: ({ contentRoot, instance }) => {
      ensureTicketStyles();
      const mount = contentRoot?.querySelector?.("[data-contact-mount]");
      if (!mount) return null;
      const key = instance?.id || instance?.instanceId || "contact-card";
      const contact = widgetDataRows(instance?.data)[0] || null;
      if (contact) lastContactByInstance.set(key, contact);
      const effective = contact || lastContactByInstance.get(key) || null;
      renderContactInto(mount, effective);
      const card = mount.closest(".widget-card");
      if (card) {
        card.onclick = null;
        card.ondblclick = effective
          ? (event) => { if (!wasDragGesture(event)) window.contactDetail?.open(effective, card); }
          : null;
        card.classList.add("db-panel-custom-color");
        card.dataset.severity = "none";
        card.dataset.contactAttention = effective && contactNeedsAttention(effective) ? "due" : "current";
        card.dataset.ticketId = effective ? (effective.id || "") : "";
      }
      return null;
    },
    render: () => `<div class="ticket-body" data-contact-mount></div>`,
  });
