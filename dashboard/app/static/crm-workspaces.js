...
(() => {
  ...
  function setActive(key){active=MODULES.some((m)=>m.key===key)?key:"home";if(!window.crmHomePreviews?.isCaptureWorker)localStorage.setItem(STORE_KEY,active);document.body.dataset.crmModule=active;try{window.crmSearchDeck?.close?.()}catch{}try{window.crmCompanyDive?.setActive?.(false)}catch{}try{window.crmRecordWorld?.close?.()}catch{}try{window.crmToday?.setActive?.(false)}catch{}try{window.crmReports?.setActive?.(false)}catch{}
    const activeApi=MODULES.find((module)=>module.key===active)?.api?.();const seen=new Set();MODULES.forEach((module)=>{try{const api=module.api?.();if(!api||seen.has(api))return;seen.add(api);const on=api===activeApi;if(apiStates.get(api)===on)return;api.setActive?.(on);apiStates.set(api,on)}catch{}});const allowed=new Set(THEATERS[active]||[]);document.querySelectorAll("[data-crm-theater]").forEach((el)=>{const hidden=!allowed.has(el.dataset.crmTheater);if(el.hidden!==hidden)el.hidden=hidden});if(root)root.hidden=active==="home";document.dispatchEvent(new CustomEvent("crm:theater-switch",{detail:{key:active}}));return active}
