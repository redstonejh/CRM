      ...
      const tick = (now) => {
        ...
        if (moving) {
          ...
          if (acrylic) {
            ...
            const owner = acrylic.parentElement;
            if (!owner || Number(owner.style.opacity || 0) < .99 || owner.style.transition.includes('opacity')) probe.acrylicUnownedFrames += 1;
            ...
            if (!probe.acrylicStyle) { const style=getComputedStyle(acrylic); probe.acrylicStyle={opacity:Number(style.opacity),backdrop:style.backdropFilter||style.webkitBackdropFilter||''}; }
        ...
        if (now - probe.startedAt < durationMs) requestAnimationFrame(tick);
          ...
          probe.result = {
            ...
            acrylicMinOpacity: probe.acrylicStyle?.opacity || 0,
            acrylicRealEveryFrame: probe.acrylicMissingFrames === 0 && probe.acrylicUnownedFrames === 0 && !!probe.acrylicStyle?.backdrop.includes('blur(8px)'),
          };
          probe.done = true;
...
function assertMotion(label, probe) {
  if (!probe || probe.cadenceFps < MOTION_TARGET.minFps || probe.p95Ms > MOTION_TARGET.maxP95Ms
    ...
    || probe.acrylicFrames < 20 || probe.acrylicMinOpacity < .99 || !probe.acrylicRealEveryFrame) {
...
async function main() {
  ...
  for (const room of rooms) {
    ...
    const acrylicMid=await page.evaluate(()=>{const surface=window.crmHomeCamera?.surface?.();const root=window.crmHomeCamera?.layers?.()[0];const expander=document.querySelector('.crm-home-expander:not(.crm-home-warm)');const edge=expander?.querySelector(':scope>.crm-home-transition-acrylic');const plane=edge;const target=root?.querySelector('.crm-home-bucket.is-camera-target');const variant=root?.querySelector(':scope>.crm-home-motion-variant.is-active-motion-variant');const snapshot=root?.querySelector(':scope>.crm-home-motion-snapshot');const buckets=[...(root?.querySelectorAll('.crm-home-grid>.crm-home-bucket')||[])];const material=(node)=>{if(!node)return null;const style=getComputedStyle(node),rect=node.getBoundingClientRect();return{opacity:Number(style.opacity),background:style.backgroundImage,backdrop:style.webkitBackdropFilter||style.backdropFilter,shadow:style.boxShadow,clip:style.clipPath,rect:[rect.x,rect.y,rect.width,rect.height]}};const selected=material(target),lid=material(expander),lens=material(edge),planeMaterial=material(plane);const nonTargets=buckets.filter((bucket)=>bucket!==target);return{frame:expander?.dataset.fractalFrame||'',selected,lid,lens,plane:planeMaterial,opacityTotal:(selected?.opacity||0)+(lid?.opacity||0),lensAligned:!!lens&&!!lid&&lens.rect.every((value,index)=>Math.abs(value-lid.rect[index])<=1),neighborAcrylic:nonTargets.length===3&&nonTargets.every((bucket)=>{const style=getComputedStyle(bucket);return(style.webkitBackdropFilter||style.backdropFilter)==='none'&&style.backgroundImage==='none'&&style.boxShadow==='none'}),objects:!!variant&&getComputedStyle(variant).display!=='none'&&variant.dataset.motionVariant===target?.dataset.module&&getComputedStyle(snapshot).display==='none'&&nonTargets.every((bucket)=>getComputedStyle(bucket.querySelector('.crm-home-preview')).visibility==='hidden')&&getComputedStyle(target?.querySelector('.crm-home-preview')).visibility==='visible',sharedWallpaper:document.querySelectorAll('body>.workspace-photo-backdrop:not([hidden])').length===1&&!expander?.querySelector('.crm-home-preview-exact')&&!!expander?.querySelector('.crm-home-preview-foreground'),acrylicLive:!!planeMaterial&&planeMaterial.backdrop.includes('blur(8px)')&&planeMaterial.background.includes('rgba(22, 26, 36, 0.62)')&&planeMaterial.opacity>.99,surfaceMoving:surface?.classList.contains('crm-home-camera-moving')}});
    ...
    const outboundAcrylic=await page.evaluate(()=>{const root=window.crmHomeCamera?.layers?.()[0];const expander=document.querySelector('.crm-home-expander:not(.crm-home-warm)');const edge=expander?.querySelector(':scope>.crm-home-transition-acrylic');const plane=edge;const target=root?.querySelector('.crm-home-bucket.is-camera-target');const variant=root?.querySelector(':scope>.crm-home-motion-variant.is-active-motion-variant');const snapshot=root?.querySelector(':scope>.crm-home-motion-snapshot');const buckets=[...(root?.querySelectorAll('.crm-home-grid>.crm-home-bucket')||[])];const material=(node)=>{if(!node)return null;const style=getComputedStyle(node),rect=node.getBoundingClientRect();return{opacity:Number(style.opacity),background:style.backgroundImage,backdrop:style.webkitBackdropFilter||style.backdropFilter,shadow:style.boxShadow,clip:style.clipPath,rect:[rect.x,rect.y,rect.width,rect.height]}};const selected=material(target),lid=material(expander),lens=material(edge),planeMaterial=material(plane);const nonTargets=buckets.filter((bucket)=>bucket!==target);return{frame:expander?.dataset.fractalFrame||'',selected,lid,lens,plane:planeMaterial,opacityTotal:(selected?.opacity||0)+(lid?.opacity||0),lensAligned:!!lens&&!!lid&&lens.rect.every((value,index)=>Math.abs(value-lid.rect[index])<=1),neighborAcrylic:nonTargets.length===3&&nonTargets.every((bucket)=>{const style=getComputedStyle(bucket);return(style.webkitBackdropFilter||style.backdropFilter)==='none'&&style.backgroundImage==='none'&&style.boxShadow==='none'}),objects:!!variant&&getComputedStyle(variant).display!=='none'&&variant.dataset.motionVariant===target?.dataset.module&&getComputedStyle(snapshot).display==='none'&&nonTargets.every((bucket)=>getComputedStyle(bucket.querySelector('.crm-home-preview')).visibility==='hidden')&&getComputedStyle(target?.querySelector('.crm-home-preview')).visibility==='visible',sharedWallpaper:document.querySelectorAll('body>.workspace-photo-backdrop:not([hidden])').length===1&&!expander?.querySelector('.crm-home-preview-exact')&&!!expander?.querySelector('.crm-home-preview-foreground'),acrylicLive:!!planeMaterial&&planeMaterial.backdrop.includes('blur(8px)')&&planeMaterial.background.includes('rgba(22, 26, 36, 0.62)')&&planeMaterial.opacity>.99}});
  ...
  if(projectDiveContinuity.frames<20||projectDiveContinuity.minOpacity<.99||!projectDiveContinuity.ownedEveryFrame||!projectDiveContinuity.realEveryFrame||projectDiveContinuity.minObjectCoverage<.94||projectDiveContinuity.maxObjectCoverage>1.08)throw new Error(`Project zoom did not retain real acrylic and one object owner on every frame: ${JSON.stringify(projectDiveContinuity)}`);
  ...
  if(projectReturnContinuity.frames<20||projectReturnContinuity.minOpacity<.99||!projectReturnContinuity.ownedEveryFrame||!projectReturnContinuity.realEveryFrame||projectReturnContinuity.minObjectCoverage<.94||projectReturnContinuity.maxObjectCoverage>1.08||projectReturnContinuity.wallpapers!==1||!projectReturnContinuity.framesSeen.includes('source'))throw new Error(`Project return did not retain real acrylic and one object owner on every frame: ${JSON.stringify(projectReturnContinuity)}`);
