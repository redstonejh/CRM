...
(() => {
  ...
  const applyZoneCardMetrics = (element) => {
    const small = isSmallObject(element); const metrics = { width:Math.round(CARD_W * (small ? SMALL_CARD_SCALE : 1)), height:Math.round(CARD_H * (small ? SMALL_CARD_SCALE : 1)) };
    if (element) { element.style.width = `${metrics.width}px`; element.style.height = `${metrics.height}px`; }
    return metrics;
  };
  ...
  const cardAccentRgb = (t) => {
    ...
    return SEV_RGB[sevOf(t)] || SEV_RGB.none;
  };
  const applyCardPaint = (element, ticket) => {
    if (!element) return;
    element.style.backgroundColor = baseColor();
    element.style.backgroundImage = cardBg(ticket);
    window.crmCardFace?.setAccent?.(element, cardAccentRgb(ticket));
  };
  ...
  const ensureStyles = () => {
    ...
    style.textContent = `
      ...
      /* Geometry/material comes from the global secondary-control primitive;
         tickets only own absolute placement, glyphs and visibility. */
      ...
      /* Titles enter the same measured content budget as every face row. */
  ...
  const cdmsReferenceFieldsHTML = (t) => {
    if (!t?.cdmsReference && t?.referenceSource !== "cdms") return "";
    const rows = [
    ...
    return rows.map(([label, value]) => {
      ...
      return text
        ? `<div class="ticket-field"><span class="ticket-field-l">${esc(label)}:</span><span class="ticket-field-v">${esc(text)}</span></div>`
        : "";
    }).join("");
  };
  const cardFieldsHTML = (t) =>
    ...
    cdmsReferenceFieldsHTML(t) + STAGE_KEYS.flatMap((k) => STAGE_FIELDS[k] || []).filter((f) => !f.prio).map((f) => {
  ...
  const cardInner = (t) => {
    ...
    return `<div class="ticket-body">` +
      ...
      `</div>` +
      (window.crmCardFace?.markup?.(t, "ticket") || "") +
  ...
  const fitCardFields = (card) => window.crmCardFace?.fit?.(card);
  ...
  const cardEl = (t, side) => {
    ...
    card.dataset.recordEntity = "tickets"; card.dataset.crmSizeKey = cardSizeKey(t.id);
  ...
  const layoutZones = () => {
    ...
    const bucketW = Math.max(168, Math.min(CARD_W + 60, (region.width - MARGIN * (n + 1)) / n));  // one full card + room for the scrollbar
    ...
    const geometry = STAGES.map((stage) => {
      const panel = zoneBody[stage.key]?.parentElement; window.crmObjectSizing?.scan?.(panel);
      const compact = isSmallObject(panel);
      return { stage, panel, width:Math.round(bucketW * (compact ? SMALL_BUCKET_SCALE : 1)), height:Math.round(bucketH * (compact ? .78 : 1)) };
    });
    const rowHeight = Math.max(...geometry.map((item) => item.height), 0);
    const totalWidth = geometry.reduce((sum, item) => sum + item.width, 0);
    const gap = Math.max(0, (region.width - totalWidth) / (n + 1));          // equal gap incl. both ends
    const startTop = zTop + Math.max(0, (availableH - rowHeight) / 2);
    let nextLeft = region.left + gap;
    const lefts = [];
    geometry.forEach(({ stage:s, panel, width, height }) => {
      const left = nextLeft;
      lefts.push(left);
      if (!panel) return;
      panel.style.top = `${Math.round(startTop + (rowHeight - height) / 2)}px`;            // fixed panels position themselves now
  ...
  const setZoneExpanded = (stage, open = !expandedStages.has(String(stage))) => {
    stage = String(stage || ""); if (!STAGE_KEYS.includes(stage)) return false;
  ...
  const zoneCardEl = (t, stage) => {
    ...
    card.dataset.id = t.id || "";
    card.dataset.recordEntity = "tickets"; card.dataset.crmSizeKey = cardSizeKey(t.id);
    window.crmObjectSizing?.scan?.(card); applyZoneCardMetrics(card);
    applyCardPaint(card, t);
  ...
  const zoneInsertIndex = (stage, y) => {
    ...
    if (expandedStages.has(stage)) {
      for (let index = 0; index < cards.length; index += 1) { const rect = cards[index].getBoundingClientRect(); if (y < rect.top + rect.height / 2) return index; }
      return count;
    }
  ...
  const previewGap = (stage, index) => {
    ...
    cards.forEach((c, i) => {
      c.style.transform = i >= index ? `translateY(${shift}px)` : "";
    });
  ...
  const restackZoneElements = (stage) => {
    ...
    const expanded = expandedStages.has(stage); let previousHeight = 0;
    [...track.querySelectorAll(":scope > .tk-zcard")].forEach((card, index) => {
      window.crmObjectSizing?.scan?.(card); const { height } = applyZoneCardMetrics(card);
      card.style.marginTop = index ? (expanded ? "10px" : `-${Math.max(0, previousHeight - ZCARD_PEEK)}px`) : "0px";
      previousHeight = height; fitCardFields(card);
    });
  ...
  const reflowObjectSizes = (detail = null) => {
    if (!theater) return; const sized = [...theater.querySelectorAll("[data-crm-size-key]")];
    if (detail?.key && !sized.some((element) => element.dataset.crmSizeKey === detail.key)) return;
    window.crmObjectSizing?.scan?.(theater); STAGES.forEach((stage) => restackZoneElements(stage.key)); layoutZones(); ["left","right","trash"].forEach((side) => layout(side));
  };
  ...
  const makeTransientSource = (ticket, anchorRect) => {
    ...
    card.style.cssText = `position:fixed;left:${left}px;top:${top}px;width:${width}px;height:${height}px;z-index:4998;`;
    applyCardPaint(card, ticket);
  ...
  window.ticketStacks = {
    ...
    createCard: (ticket, options = {}) => {
      ...
      card.dataset.id = ticket?.id || "";
      card.dataset.recordEntity = "tickets"; card.dataset.crmSizeKey = cardSizeKey(ticket?.id);
      window.crmObjectSizing?.scan?.(card);
      card.style.width = `${CARD_W}px`; card.style.height = `${CARD_H}px`;
      applyCardPaint(card, ticket);
      card.innerHTML = cardInner(ticket);
