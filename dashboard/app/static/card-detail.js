// card-detail.js — reusable context-aware card detail flyout.
//
// The original ticket detail view proved the motion: left-click a card → a flat
// opaque clone glides from the card's spot in a
...
global.createCrmCardDetail = function createCrmCardDetail(config = {}) {
  ...
  const PANEL_W = Math.max(320, Math.min(440, Number(config.panelWidth) || 360));
  ...
  const ensureStyles = () => {
    if (document.getElementById("crm-card-detail-styles")) return;
    const style = document.createElement("style");
    style.id = "crm-card-detail-styles";
    style.textContent = `
  ...
  const buildStage = (ticket) => {
    ...
    // Flyer destination from the motion. The vertical seat is finalized after
    // the panel's complete, natural height has been measured.
    ...
    // Render once at its final width, offstage, so every field and auto-grown
    // textarea participates in the measurement. The settled menu never owns a
    // scrollbar: its complete content determines its height.
  ...
  const ticketById = async (id) => {
    try { const r = await getSource()?.list?.(); return recordsFromList(r).find((t) => t.id === id) || null; }
    catch { return null; }
  };
  ...
  const render = (t) => {
    ...
    const sf = (getStacks()?.stageFields?.(t.id)) || { key: "triage", label: "Triage", fields: [] };
  ...
  const wire = (t) => {
    ...
    panel.querySelectorAll(".td-prio-opt").forEach((el) => {
      el.onclick = () => {
        ...
        getStacks()?.setPriority?.(t.id, val);
    ...
    const grow = (el) => { if (el.tagName === "TEXTAREA") { el.style.height = "auto"; el.style.height = `${el.scrollHeight}px`; fitPanelToContent(); } };
  ...
  const open = (ticket, srcEl, opts) => {
    ...
    settleTimer = setTimeout(() => {
      ...
      panel.style.transition = "box-shadow .25s ease";   // let the shadow fade in; dimensions and transform are already at rest
    ...
    if (!subscribed) { subscribed = true; getSource()?.onChanged?.(() => refresh()); }
  };
