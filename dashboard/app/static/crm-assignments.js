...
(() => {
  ...
  function ensureStyles() {
    ...
    const style = document.createElement("style"); style.id = "crm-assignments-styles"; style.textContent = `
      ...
      .crm-assignment-pipeline{min-width:0;min-height:0;display:flex;align-items:flex-start;gap:14px;overflow-x:auto;overflow-y:hidden;padding:0 4px 22px;box-sizing:border-box;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.18) transparent}
      .crm-assignment-bucket.tk-zone{position:relative;inset:auto;z-index:auto;flex:0 0 206px;width:206px;height:min(600px,calc(100vh - 184px));min-height:390px;box-sizing:border-box;padding:12px 13px 13px;overflow:hidden;transition:width .18s cubic-bezier(.22,1,.26,1),flex-basis .18s cubic-bezier(.22,1,.26,1),height .18s cubic-bezier(.22,1,.26,1)}
      ...
      .crm-assignment-work-card{appearance:none;position:relative;flex:0 0 auto;width:176px;min-height:116px;box-sizing:border-box;padding:12px 13px;text-align:left;border:0;border-radius:15px;color:rgba(255,255,255,.9);background:linear-gradient(150deg,rgba(92,108,131,.95),rgba(54,67,87,.94));box-shadow:inset 0 1px rgba(255,255,255,.22),0 14px 18px -14px rgba(0,0,0,.55);cursor:grab;overflow:hidden;transition:width .18s cubic-bezier(.22,1,.26,1),min-height .18s cubic-bezier(.22,1,.26,1),margin .2s cubic-bezier(.22,1,.26,1),opacity .14s ease,box-shadow .14s ease}.crm-assignments-surface.is-seating .crm-assignment-work-card{transition:none!important}.crm-assignment-work-card+.crm-assignment-work-card{margin-top:-72px}.crm-assignment-card-list.is-expanded .crm-assignment-work-card+.crm-assignment-work-card{margin-top:0}.crm-assignment-work-card[data-priority="high"]{background:linear-gradient(150deg,rgba(133,104,83,.96),rgba(78,63,57,.94))}.crm-assignment-work-card[data-priority="urgent"]{background:linear-gradient(150deg,rgba(142,82,82,.96),rgba(81,50,59,.94))}.crm-assignment-work-card:hover,.crm-assignment-work-card:focus-visible{outline:0;box-shadow:inset 0 0 0 9999px rgba(255,255,255,.09),inset 0 1px rgba(255,255,255,.3),0 16px 22px -14px rgba(0,0,0,.6)}.crm-assignment-work-card.is-dragging{opacity:.3}.crm-assignment-work-card:active{cursor:grabbing}
      ...
      @media(max-width:1250px){.crm-assignments-frame{grid-template-columns:172px minmax(0,1fr);gap:14px}.crm-assignment-bucket.tk-zone{flex-basis:190px;width:190px}.crm-assignment-work-card{width:162px}}
  ...
  const api = { setActive, baseline, miniature, refresh, move, assign, unassign, create:() => openEditor(), open:(id, anchor) => { const item = itemById(id); if (!item) return false; openEditor(item, anchor); return true; }, items:() => clone(model.commitments), stages:() => clone(STAGES), selectFilter:(id) => { selectedFilter = String(id); render(); }, setStageExpanded, expandedStages:() => [...expandedStages], isActive:() => active };
