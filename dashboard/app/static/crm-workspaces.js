...
(() => {
  ...
  function setActive(key){active=MODULES.some((m)=>m.key===key)?key:"home";if(!window.crmHomePreviews?.isCaptureWorker)localStorage.setItem(STORE_KEY,active);document.body.dataset.crmModule=active;try{window.crmSearchDeck?.close?.()}catch{}try{window.crmCompanyDive?.setActive?.(false)}catch{}try{window.crmRecordWorld?.close?.()}catch{}try{window.crmToday?.setActive?.(false)}catch{}try{window.crmReports?.setActive?.(false)}catch{}try{window.peopleCards?.setActive?.(false)}catch{}
    const activeApi=MODULES.find((module)=>module.key===active)?.api?.();const seen=new Set();MODULES.forEach((module)=>{try{const api=module.api?.();if(!api||seen.has(api))return;seen.add(api);api.setActive?.(api===activeApi)}catch{}});const allowed=new Set(THEATERS[active]||[]);document.querySelectorAll("[data-crm-theater]").forEach((el)=>{el.hidden=!allowed.has(el.dataset.crmTheater)});if(root)root.hidden=active==="home";document.dispatchEvent(new CustomEvent("crm:theater-switch",{detail:{key:active}}));return active}
