...
(() => {
  ...
  function ensureStyles() {
    ...
    const style = document.createElement("style"); style.id = "crm-assignments-styles"; style.textContent = `
      ...
      .crm-assignments-frame{position:absolute;inset:var(--crm-canvas-top,78px) var(--crm-canvas-x,64px) var(--crm-canvas-bottom,78px);display:grid;grid-template-rows:46px minmax(0,1fr);gap:12px;min-width:0;min-height:0}
      .crm-assignment-tabs{min-width:0;height:46px;display:flex;align-items:center;justify-content:space-between;-webkit-app-region:no-drag}.crm-assignment-title{flex:0 0 auto;font-size:var(--crm-type-room,17px);font-weight:700;letter-spacing:-.01em}.crm-assignment-new{flex:0 0 auto}
      ...
      .crm-assignment-bucket .tk-zone-hd{flex:0 0 30px}
      ...
      .crm-assignment-card-progress{position:absolute;right:13px;top:14px;display:flex;gap:3px}.crm-assignment-card-progress i{display:block;width:8px;height:3px;border-radius:4px;background:rgba(255,255,255,.18)}.crm-assignment-card-progress i.is-past{background:rgba(94,231,157,.8)}.crm-assignment-card-title{max-width:132px;font-size:var(--crm-type-object,14px);font-weight:800;line-height:1.22}.crm-assignment-card-note{margin-top:9px;color:rgba(255,255,255,.67);font-size:var(--crm-type-body,12px);line-height:1.38}.crm-assignment-card-details{display:grid;gap:7px;margin-top:auto;padding-top:13px}.crm-assignment-card-meta{display:grid;grid-template-columns:48px minmax(0,1fr);gap:6px;color:rgba(255,255,255,.75);font-size:var(--crm-type-meta,10px);line-height:1.25}.crm-assignment-card-meta b{color:rgba(255,255,255,.4);font-weight:700}
  ...
  const assignmentAccent = (item) => ({
    high:"202, 138, 4",
    urgent:"220, 38, 38",
    normal:"14, 165, 233",
  })[String(item?.priority || "normal").toLowerCase()] || "14, 165, 233";
  const fitAssignmentCard = (card, item = itemById(card?.dataset?.assignmentCard)) => {
    if (!card) return false;
    window.crmCardPresentation?.setAccent?.(card, assignmentAccent(item));
    return window.crmCardPresentation?.fit?.(card) ?? false;
  };
  const fitAssignmentCards = (scope = root) => {
    scope?.querySelectorAll?.("[data-assignment-card]").forEach((card) => fitAssignmentCard(card));
  };
  function cardHTML(item) {
    ...
    const mark = window.crmCardPresentation?.markup?.(item, "task") || "";
    return `<button type="button" class="crm-assignment-work-card crm-semantic-card" draggable="true" data-assignment-card="${esc(item.id)}" data-card-detail-card data-record-entity="commitments" data-record-id="${esc(item.id)}" data-crm-size-key="${esc(`card:commitments:${item.id}`)}" data-priority="${esc(String(item.priority || "normal").toLowerCase())}" aria-label="${esc(first(item.title, "Untitled assignment"))}"><span class="crm-assignment-card-face ticket-body" data-card-fit-body>${mark}<span class="crm-card-face-content"><span class="crm-assignment-card-progress" aria-hidden="true">${progress}</span><span class="crm-assignment-card-title" data-card-fit-entry>${esc(first(item.title, "Untitled assignment"))}</span>${first(item.context, item.note, item.description) ? `<span class="crm-assignment-card-note" data-card-fit-entry>${esc(first(item.context, item.note, item.description))}</span>` : ""}<span class="crm-assignment-card-details" data-card-fit-fields><span class="crm-assignment-card-meta"><b>Owner</b><span data-card-fit-entry>${esc(first(item.assignee, "Unassigned"))}</span></span><span class="crm-assignment-card-meta"><b>For</b><span data-card-fit-entry>${esc(contextLabel(item))}</span></span><span class="crm-assignment-card-meta"><b>Priority</b><span data-card-fit-entry>${esc(first(item.priority, "Normal"))}</span></span></span></span></span></button>`;
  }
  ...
  function routeBoardWheel(event) {
    if (event.defaultPrevented) return false;
    const raw = Math.abs(event.deltaX) > Math.abs(event.deltaY) ? event.deltaX : event.deltaY; if (!raw) return false;
    const width = boardElements().clip?.clientWidth || innerWidth;
    const pixels = event.deltaMode === 1 ? raw * 16 : event.deltaMode === 2 ? raw * width : raw;
    if (!scrollBoardBy(pixels)) return false;
  ...
  function wireBoardScroller() {
    ...
    clip.addEventListener("wheel", routeBoardWheel, { passive:false });
  ...
  function render() {
    ...
    root.innerHTML = `<div class="crm-assignments-frame"><header class="crm-assignment-tabs"><span class="crm-assignment-title">Assignments</span><button type="button" class="crm-assignment-new crm-secondary-control" data-assignment-action="new" aria-label="Create assignment" title="Create assignment"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><path d="M12 5v14M5 12h14"></path></svg></button></header><section class="crm-assignment-board" aria-label="Assignment pipeline"><div class="crm-assignment-board-clip" tabindex="0" aria-label="Scrollable assignment buckets"><div class="crm-assignment-pipeline">${STAGES.map((stage) => {
  ...
  const patchAssignmentCard = (item) => {
    ...
    document.querySelectorAll(`[data-assignment-card="${String(item.id).replace(/["\\\]]/g, "\\$&")}"]`).forEach((card) => {
      ...
      requestAnimationFrame(() => fitAssignmentCard(card, item));
    });
  };
  ...
  const setStageExpanded = (stageId, open = !expandedStages.has(expansionKey(stageId))) => {
    ...
    const bucket = root?.querySelector(`[data-assignment-stage="${key}"]`); const list = bucket?.querySelector(".crm-assignment-card-list");
  ...
  function wire() {
    ...
    root.addEventListener("wheel", (event) => {
      ...
      const clip = boardElements().clip?.getBoundingClientRect();
